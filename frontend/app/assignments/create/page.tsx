...
import { UploadCloud, Calendar, Plus, Mic, ArrowLeft, ArrowRight, ChevronDown, Minus, X } from 'lucide-react';
import { useAssessmentStore } from '@/store/useAssessmentStore';

export default function CreateAssignmentPage() {
  const router = useRouter();
  const { formData, updateFormData, addQuestionType, updateQuestionType, removeQuestionType } = useAssessmentStore();
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto relative">
      ...
      <div className="md:hidden flex items-center gap-3 px-4 py-3 border-b border-[#F3F4F6]">
        ...
        <h2 className="text-[15px] font-semibold text-[#303030]">Create Assignment</h2>
      </div>
      ...
      <p className="hidden md:block text-[13px] text-[#9CA3AF] ml-6">Set up a new assignment for your students</p>
      ...
      <div className="flex-1 px-4 md:px-8 pt-4 md:pt-8 pb-36 md:pb-32 max-w-[900px] mx-auto w-full relative">
        ...
        <div className="bg-white md:bg-white/50 mx-auto rounded-[20px] md:rounded-[32px] p-5 md:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] mb-8">
          <h3 className="text-[16px] md:text-[20px] font-bold text-[#303030] font-bricolage mb-0.5">Assignment Details</h3>
          <p className="text-[12px] md:text-[14px] text-[#5E5E5E] mb-5 md:mb-8">Basic information about your assignment</p>

          {/* File Upload */}
          <div className="border-2 border-dashed border-[#EAEAEA] rounded-[16px] md:rounded-[24px] py-6 md:py-10 px-4 md:px-6 flex flex-col items-center justify-center bg-white mb-2 md:mb-4 hover:bg-[#FAFAFA] transition-colors cursor-pointer group">
            <UploadCloud size={20} className="md:w-6 md:h-6 text-[#303030] mb-2 md:mb-4" strokeWidth={2} />
            <p className="font-semibold text-[13px] md:text-[16px] text-[#303030] mb-0.5 text-center">Choose a file or drag & drop it here</p>
            <p className="text-[11px] md:text-[14px] text-[#A9A9A9] mb-3 md:mb-5">JPEG, PNG, upto 10MB</p>
            <button className="px-5 md:px-6 py-1.5 md:py-2 bg-[#F5F5F5] hover:bg-[#EEEEEE] rounded-full text-[12px] md:text-[14px] font-semibold text-[#303030] transition-colors">
              Browse Files
            </button>
          </div>
          <p className="text-center text-[11px] md:text-[14px] text-[#303030]/60 mb-5 md:mb-8">Upload images of your preferred document/image</p>

          {/* Due Date */}
          <div className="mb-5 md:mb-8">
            <label className="block text-[13px] md:text-[16px] font-bold text-[#303030] mb-2 md:mb-3">Due Date</label>
            <div className="relative">
              <input
                type="text"
                placeholder="DD-MM-YYYY"
                className="w-full pl-4 md:pl-6 pr-10 md:pr-12 py-2.5 md:py-3.5 bg-[#F9FAFB] border border-[#EAEAEA] rounded-full text-[13px] md:text-[14px] outline-none focus:border-[#D1D5DB] focus:bg-white transition-colors placeholder:text-[#9CA3AF] font-medium text-[#303030]"
                value={formData.dueDate}
                onChange={(e) => updateFormData({ dueDate: e.target.value })}
              />
              <Calendar size={16} className="md:w-[18px] md:h-[18px] absolute right-4 md:right-6 top-1/2 -translate-y-1/2 text-[#303030]" />
            </div>
          </div>
          ...
          <div className="mb-6 md:mb-10">
            ...
            <div className="md:hidden mb-3">
              <label className="text-[13px] font-bold text-[#303030]">Question Type</label>
            </div>
              ...
              {formData.questionTypes.map((qt) => (
                <div key={qt.id}>
                  ...
                  <div className="hidden md:grid grid-cols-12 gap-4 items-center">
                    ...
                    <div className="col-span-1 text-center text-[#303030] font-bold text-[16px]">x</div>
                    <div className="col-span-2 flex items-center justify-between bg-white border border-[#EAEAEA] rounded-full px-4 py-3.5 shadow-sm shadow-black/5">
                      <button onClick={() => updateQuestionType(qt.id, { count: Math.max(1, qt.count - 1) })} className="text-[#D1D5DB] hover:text-[#303030] transition-colors"><Minus size={16} strokeWidth={3} /></button>
                      <span className="font-bold text-[14px] text-[#303030]">{qt.count}</span>
                      <button onClick={() => updateQuestionType(qt.id, { count: qt.count + 1 })} className="text-[#D1D5DB] hover:text-[#303030] transition-colors"><Plus size={16} strokeWidth={3} /></button>
                    </div>
                    <div className="col-span-1 text-center text-[#303030] font-bold text-[16px] opacity-0 pointer-events-none">x</div>
                    <div className="col-span-2 flex items-center justify-between bg-white border border-[#EAEAEA] rounded-full px-4 py-3.5 shadow-sm shadow-black/5">
                      <button onClick={() => updateQuestionType(qt.id, { marks: Math.max(1, qt.marks - 1) })} className="text-[#D1D5DB] hover:text-[#303030] transition-colors"><Minus size={16} strokeWidth={3} /></button>
                      <span className="font-bold text-[16px] text-[#303030]">{qt.marks}</span>
                      <button onClick={() => updateQuestionType(qt.id, { marks: qt.marks + 1 })} className="text-[#D1D5DB] hover:text-[#303030] transition-colors"><Plus size={16} strokeWidth={3} /></button>
                    </div>
                  </div>
                  ...
                  <div className="md:hidden bg-[#F9FAFB] border border-[#EAEAEA] rounded-[16px] p-4">
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="flex-1">
                        <label className="text-[10px] text-[#9CA3AF] font-semibold mb-1.5 block">No. of Questions</label>
                        <div className="flex items-center justify-between bg-white border border-[#EAEAEA] rounded-full px-3 py-2">
                          <button onClick={() => updateQuestionType(qt.id, { count: Math.max(1, qt.count - 1) })} className="text-[#D1D5DB] hover:text-[#303030]"><Minus size={12} strokeWidth={3} /></button>
                          <span className="font-bold text-[13px] text-[#303030]">{qt.count}</span>
                          <button onClick={() => updateQuestionType(qt.id, { count: qt.count + 1 })} className="text-[#D1D5DB] hover:text-[#303030]"><Plus size={12} strokeWidth={3} /></button>
                        </div>
                      </div>
                      {/* Marks */}
                      <div className="flex-1">
                        <label className="text-[10px] text-[#9CA3AF] font-semibold mb-1.5 block">Marks</label>
                        <div className="flex items-center justify-between bg-white border border-[#EAEAEA] rounded-full px-3 py-2">
                          <button onClick={() => updateQuestionType(qt.id, { marks: Math.max(1, qt.marks - 1) })} className="text-[#D1D5DB] hover:text-[#303030]"><Minus size={12} strokeWidth={3} /></button>
                          <span className="font-bold text-[13px] text-[#303030]">{qt.marks}</span>
                          <button onClick={() => updateQuestionType(qt.id, { marks: qt.marks + 1 })} className="text-[#D1D5DB] hover:text-[#303030]"><Plus size={12} strokeWidth={3} /></button>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            >
              <div className="bg-[#2B2B2B] text-white rounded-full w-[28px] h-[28px] md:w-[36px] md:h-[36px] flex items-center justify-center group-hover:bg-black transition-colors">
                <Plus size={12} className="md:w-[14px] md:h-[14px]" strokeWidth={2.5} />
              </div>
              Add Question Type
            </button>
      ...
      <div className="fixed bottom-16 md:bottom-8 left-4 right-4 md:left-[340px] md:right-12 z-20 pointer-events-none">
        <div className="max-w-[900px] mx-auto w-full px-0 md:px-8 flex justify-between pointer-events-auto gap-3">
          ...
          <button
            onClick={handleNext}
            className="flex items-center gap-1.5 md:gap-2 bg-[#303030] hover:bg-black text-white px-5 md:px-8 py-2.5 md:py-3.5 rounded-full text-[12px] md:text-[14px] font-bold transition-colors shadow-[0_8px_30px_rgb(0,0,0,0.12)]"
          >
            Next
