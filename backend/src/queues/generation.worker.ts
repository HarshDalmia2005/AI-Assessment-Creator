    ...
    async (job) => {
      ...
      try {
        ...
        if (ioInstance) {
          ioInstance
            .to(assignmentId)
            .emit("status_update", { status: "generating" });
        }
        ...
        // Save result
        await Assignment.findByIdAndUpdate(assignmentId, {
          title: generatedPaper.title || "AI Generated Assessment",
          status: "completed",
          generatedPaper,
        });
        ...
        if (ioInstance) {
          ioInstance
            .to(assignmentId)
            .emit("generation_complete", { assignmentId });
        }
        ...
        if (ioInstance) {
          ioInstance
            .to(assignmentId)
            .emit("status_update", { status: "failed" });
        }
      }
    },
